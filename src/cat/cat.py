...
from cat import CLUSTER_FIELD, MIN_N_GENES, N_ITERATIONS
...
def internal_preprocessing(
    ...
    if not (check_normalization(ds1) or check_normalization(ds2)):
        logging.error("Datasets are not normalized properly. Make sure you normalized data in advance.")
        sys.exit(1)
...
def compare(
    ...
    df = pl.DataFrame(normalize(adata.X, method="median"), schema=adata.var_names.tolist()).with_columns(
        pl.Series(name=CLUSTER_FIELD, values=adata.obs[CLUSTER_FIELD])
    )

    distances = []
    partitions: dict[str, pl.DataFrame] = df.partition_by(CLUSTER_FIELD, as_dict=True, include_key=False)
    cluster_names = list(sum(partitions.keys(), ()))

    for _ in track(range(n_iterations), description="Processing..."):
        # FIFTH - CLUSTER AVERAGES
        cluster_means = pl.concat([x.sample(fraction=1.0, with_replacement=True).mean() for x in partitions.values()])
    ...
    dist_mean_df = pl.DataFrame(squareform(dist_mean), schema=cluster_names).with_columns(
